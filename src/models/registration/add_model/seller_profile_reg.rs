...
pub fn result(
    ...
    if doc_hash.is_some() {
        ...
        if object_id.is_ok() {
            ...
            let instance = sellers::SellerProfile::find_one_to_model_instance(filter, None)?;
            if instance.is_some() {
        ...
        // Delete document
        // -----------------------------------------------------------------------------------------
        let instance = sellers::SellerProfile::find_one_to_model_instance(filter.unwrap(), None)?;
        if instance.is_some() {
