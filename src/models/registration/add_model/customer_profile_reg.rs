...
use crate::{models::services::accounts::customers, settings};

const MODUL_NAME: &str = "customer_profile_reg";
...
pub fn result(
    ...
    if doc_hash.is_some() {
        ...
        if object_id.is_ok() {
            ...
            let instance = customers::CustomerProfile::find_one_to_model_instance(filter, None)?;
            if instance.is_some() {
            ...
            return customers::CustomerProfile::model_to_json_for_admin();
        }
    } else if bytes.is_some() && app_state.is_some() {
        // Save document
        // -----------------------------------------------------------------------------------------
        //let app_state = app_state.unwrap();
        let mut instance = serde_json::from_slice::<customers::CustomerProfile>(&bytes.unwrap())?;
        //
        //
        ...
        // -----------------------------------------------------------------------------------------
        let instance =
            customers::CustomerProfile::find_one_to_model_instance(filter.unwrap(), None)?;
        if instance.is_some() {
        ...
        // -----------------------------------------------------------------------------------------
        customers::CustomerProfile::db_update_dyn_widgets(options_json.unwrap())?;
    } else {
        // Error
        // -----------------------------------------------------------------------------------------
        Err(format!(
